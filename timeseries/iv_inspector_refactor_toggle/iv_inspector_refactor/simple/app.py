...
APP_CACHE_DIR = PROJECT_ROOT / "data" / "cache" / "iv_app_simple"
...
def sidebar_params():
    with st.sidebar:
        ...
        if fp:
            fp_hint = fp if Path(fp).is_absolute() else str((PROJECT_ROOT / fp).resolve())
            try:
...
def _aggregation_cache_path(params: dict) -> Path:
    ...
    src_abs = (PROJECT_ROOT / src).resolve() if not src.is_absolute() else src.resolve()
...
def main():
    ...
    if fp and not Path(fp).is_absolute():
        fp = str((PROJECT_ROOT / fp).resolve())
    df_raw = load_data(fp)
