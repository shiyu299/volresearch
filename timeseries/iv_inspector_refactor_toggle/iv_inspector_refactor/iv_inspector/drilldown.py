...
def make_detail_table(df_interval: pd.DataFrame, symbols: list[str]) -> pd.DataFrame:
    ...
    cols = []
    for c in [
        "dt_exch", "symbol", "underlying", "cp", "K", "F_used", "T",
        "iv", "mid", "spread",
        "bidprice1", "bidvol1","askprice1","askvol1", "lastprice",
        "trade_price", "has_trade", "trade_sign",
        "traded_vega", "traded_vega_signed",
        "d_volume", "trade_volume_lots", "volume",
...
def render_drilldown_tabs(
    *,
    ...
    tab1, tab2, tab3 = st.tabs([f"ATM±{n_main}（OTM+ATM）", f"ATM±{n_alt}（OTM+ATM）", f"TopVol {top_m}"])
